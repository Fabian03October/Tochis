{{-- 
    Tarjeta de orden para 'Listas'.
    La tarjeta es blanca y el botón de acción es 'btn-success'.
--}}
<div class="bg-white rounded-lg shadow border border-gray-200 m-2 p-3 transition-all hover:shadow-md">
    ...
    <div class="flex justify-between items-center mb-2">
        ...
        <div class="flex items-center space-x-2">
            @if($order->station_type === 'bar')
                <span class="inline-flex px-2 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">Barra</span>
            @else
                <span class="inline-flex px-2 text-xs font-semibold rounded-full bg-gray-100 text-gray-800">Cocina</span>
            @endif
    ...
    {{-- Botón de Acción --}}
    <button class="btn-success w-full mt-3" onclick="markReceived({{ $order->id }})">
